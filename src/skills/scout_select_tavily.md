# Scout Select Prompt — Tavily backend (sent to DeepSeek)
...
Each entry shows a real page title (Title) and a content snippet (Info).
...
- Generic shared words (capital / investment / insurance / asia / group / holdings) do not indicate the same entity — judge by the core brand identifier in the business description
- Multiple companies may share a similar name; judge entity identity from title and snippet content

**On page titles (Title field):**
- The Title here is the real HTML page title of the site — use it together with the snippet as evidence for entity attribution
- A company name appearing in a title does not confirm it is the official site — directory and news pages also frequently contain the company name in their title
...
{{"reason": "<one sentence citing title and snippet evidence>", "official_index": <integer or null>}}
